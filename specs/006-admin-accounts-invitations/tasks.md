...
- [ ] T001 Add `nodemailer` (+ `@types/nodemailer` dev dep) to `apps/backend/package.json` per
      research.md #1; run `npm install`
- [ ] T002 [P] Generate `libs/domain/invitations` lib via Nx (`@nx/js:lib`, matching
      `libs/domain/auth`'s buildable/testable config in `libs/domain/auth/{package.json,tsconfig*.json,jest.config.cts}`)
- [ ] T003 [P] Add `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_FROM`,
...
- [ ] T004 [P] Write failing tests for `canRemoveLastAdmin(activeAdminCount, isTargetActiveAdmin)` in
...
- [ ] T005 [P] Implement `libs/domain/auth/src/lib/last-admin.ts` to pass T004; export from
      `libs/domain/auth/src/index.ts`
- [ ] T006 [P] Write failing tests for token generation/expiry in
...
- [ ] T007 [P] Implement `libs/domain/invitations/src/lib/invitation-token.ts` to pass T006
- [ ] T008 [P] Write failing tests for the state machine in
      ...
      Lifecycle)
- [ ] T009 [P] Implement `libs/domain/invitations/src/lib/invitation-state.ts` to pass T008; export
      both from `libs/domain/invitations/src/index.ts`

### Schema & repositories

- [ ] T010 Extend `apps/backend/src/database/database.service.ts` `migrateAuth()` (or equivalent
...
- [ ] T011 Add `invitations` table creation to `apps/backend/src/database/database.service.ts`
...
- [ ] T012 Extend `apps/backend/src/auth/users.repository.ts`: `findAll()` (all accounts, any
...
- [ ] T013 [P] Write/extend `apps/backend/src/auth/users.repository.spec.ts` for the T012 additions
...
- [ ] T014 Create `apps/backend/src/invitations/invitations.repository.ts`: `create(...)`,
...
- [ ] T015 [P] Write `apps/backend/src/invitations/invitations.repository.spec.ts` against a real
...
- [ ] T016 [P] Create `libs/api-contract/src/lib/accounts.ts` per contracts/accounts-api.md
      (`AccountSummary`, `ChangeRoleRequest`, `AccountsErrorResponse`); export from
      `libs/api-contract/src/index.ts`
- [ ] T017 [P] Create `libs/api-contract/src/lib/invitations.ts` per contracts/invitations-api.md
...
- [ ] T018 [P] [US1] Integration test in
...
- [ ] T019 [P] [US1] Integration test: `PATCH /api/accounts/:id/role` — 200 + role takes effect on
...
- [ ] T020 [P] [US1] Integration test: `POST /api/accounts/:id/archive` — 200 + status ARCHIVED +
      ...
      SC-004)
- [ ] T021 [P] [US1] Integration test: `POST /api/accounts/:id/reactivate` — 200 + status ACTIVE +
      original data intact, 404 already-active/unknown, 410 retention_expired past window
      (Scenario A.4, Acceptance #3, FR-003)
- [ ] T022 [P] [US1] Integration test: `DELETE /api/accounts/:id` — 204 self-delete + sessions
      invalidated, 403 on non-self id, 409 last-admin (Scenario A.6, Acceptance #6, FR-004)
- [ ] T023 [P] [US1] Integration test: non-admin (`MEMBER` role session) calling any
      `/api/accounts/*` route → 403 regardless of route (Scenario A.5, Acceptance #4, FR-006)

### Implementation for User Story 1

- [ ] T024 [US1] Create `apps/backend/src/accounts/accounts.module.ts` (imports `DatabaseModule`,
      `AuthModule`)
- [ ] T025 [US1] Create `apps/backend/src/accounts/accounts.service.ts`: `listAll()` (maps
...
- [ ] T026 [US1] Create `apps/backend/src/accounts/accounts.controller.ts`: `GET /`, `PATCH
...
- [ ] T027 [US1] Register `AccountsModule` in `apps/backend/src/app/app.module.ts`
- [ ] T028 [US1] Confirm T018–T023 pass (Red→Green); add logging via `JsonLoggerService` for
      role-change/archive/reactivate/delete (actor, target, outcome — no password/token values,
      Principle V)
- [ ] T029 [P] [US1] Create `apps/frontend/src/app/settings/accounts/accounts.component.ts` +
...
- [ ] T030 [P] [US1] Create `apps/frontend/src/app/settings/accounts/archive-confirm-dialog` (or
      inline PrimeNG `ConfirmDialog`) naming the retention window and immediate sign-out
      (design.md "Archive-confirm dialog")
- [ ] T031 [US1] Wire `AccountsComponent` into `apps/frontend/src/app/settings/settings.component.ts`
      as the "Accounts" sub-tab (matches existing `HealthStatusComponent` sub-tab pattern)
- [ ] T032 [US1] Add an accounts API client (e.g. `apps/frontend/src/app/settings/accounts/accounts.service.ts`)
...
- [ ] T033 [P] [US2] Integration test in
...
- [ ] T034 [P] [US2] Integration test: second invite to same email supersedes the first — new row
      201, old row status SUPERSEDED, old token 410s on token-lookup (Scenario B.3, Acceptance #4,
      FR-009)
- [ ] T035 [P] [US2] Integration test: `GET /api/invitations` lists all statuses with
      status/createdAt (Scenario B.2, Acceptance #2, FR-010)
- [ ] T036 [P] [US2] Integration test: `POST /api/invitations/:id/cancel` — 200 CANCELLED, 404
      unknown, 409 already_resolved on non-pending/race (Scenario B.7, FR-010, FR-012)
- [ ] T037 [P] [US2] Integration test: `POST /api/invitations/:id/resend` — 201 new row + old
      superseded + re-emailed, 404/409/502 paths (FR-009/FR-010)
- [ ] T038 [P] [US2] Integration test: `GET /api/invitations/token/:token` — 200 with
      ...
      (Acceptance #6, FR-012)
- [ ] T039 [P] [US2] Integration test: `POST /api/invitations/token/:token/accept` — 201 + user
...
- [ ] T040 [US2] Create `apps/backend/src/invitations/email.service.ts`: `nodemailer` SMTP
...
- [ ] T041 [US2] Create `apps/backend/src/invitations/invitations.module.ts` (imports
      `DatabaseModule`, `AuthModule`)
- [ ] T042 [US2] Create `apps/backend/src/invitations/invitations.service.ts`:
      ...
      mechanism)
- [ ] T043 [US2] Create `apps/backend/src/invitations/invitations.controller.ts`: admin routes
...
- [ ] T044 [US2] Register `InvitationsModule` in `apps/backend/src/app/app.module.ts`
- [ ] T045 [US2] Confirm T033–T039 pass (Red→Green); add logging via `JsonLoggerService` for
      send/cancel/resend/accept (actor, target, outcome — no token value ever logged, Principle V)
- [ ] T046 [US2] Create `apps/backend/src/accounts/retention-sweep.service.ts` (or similar,
...
- [ ] T047 [P] [US2] Write test for the retention sweep in
...
- [ ] T048 [P] [US2] Create `apps/frontend/src/app/settings/invitations/invitations.component.ts` +
...
- [ ] T049 [P] [US2] Create `apps/frontend/src/app/settings/invitations/invite-dialog` component:
...
- [ ] T050 [US2] Wire `InvitationsComponent` into `apps/frontend/src/app/settings/settings.component.ts`
      as the "Invitations" sub-tab
- [ ] T051 [P] [US2] Add an invitations API client (e.g.
...
- [ ] T052 [P] [US2] Create `apps/frontend/src/app/invite/accept/accept.component.ts` + `.html`/`.css`:
...
- [ ] T053 [P] [US2] Create `apps/frontend/src/app/invite/expired/expired.component.ts` + `.html`/`.css`:
      shell-less neutral messaging page for used/cancelled/expired links (design.md
      "Invite-expired/used page")
- [ ] T054 [US2] Add `/invite/:token` public routes to `apps/frontend/src/app/app.routes.ts` (accept
...
- [ ] T055 [P] Run `apps/backend/src/database/database.service.spec.ts` (or add coverage) confirming
...
- [ ] T056 [P] Add unit tests for `AccountsService`'s `isLastActiveAdmin` computation and
...
- [ ] T057 Execute quickstart.md Scenario A and Scenario B end-to-end against the Docker Compose
      ...
      status code/body
- [ ] T058 [P] Verify no password or invitation token value appears in any `JsonLoggerService`
      output across accounts/invitations flows (Principle V)
- [ ] T059 Run `npm exec nx run-many -t lint test build -p backend frontend api-contract` (or
