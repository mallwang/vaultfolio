...
- [x] T001 Create `libs/domain/accounts` Nx library (package.json, project.json, tsconfig\*.json,
...
- [x] T002 [P] Define `AccountCategory` union + `ACCOUNT_CATEGORIES` ordered list in
...
- [x] T003 [P] Define `Account` entity class in `libs/domain/accounts/src/lib/account.ts` (all
...
- [x] T004 [US-shared] Implement `account-validation.ts` in
...
- [x] T005 [P] Unit tests for `account-validation.ts` in
...
- [x] T006 [P] Unit tests for the `Account` entity in `libs/domain/accounts/src/lib/account.spec.ts`
      (construction/shape), mirroring `holding.spec.ts` (depends on T003)
- [x] T007 Export `AccountCategory`, `ACCOUNT_CATEGORIES`, `Account`, validation functions/types
      through `libs/domain/accounts/src/index.ts` (depends on T002-T004)
- [x] T008 [P] Define shared contract types in `libs/api-contract/src/lib/account-overview.ts`:
...
- [x] T009 Add the `accounts` table + `accounts_owner_id_idx` index to
...
- [x] T010 [P] Implement `account-overview.mapper.ts` in
...
- [x] T011 Implement `account-overview.repository.ts` in
...
- [x] T012 [P] Repository unit tests in
...
- [x] T013 Implement `account-overview.service.ts` in
...
- [x] T014 Implement `account-overview.controller.ts` in
...
- [x] T015 [P] Controller unit tests in
      ...
      (depends on T014)
- [x] T016 Implement `account-overview.module.ts` in
...
- [x] T017 Register `AccountOverviewModule` in `apps/backend/src/app/app.module.ts` alongside
...
- [x] T018 [P] [US1] Implement `account-overview.service.ts` (frontend HTTP client) in
...
- [x] T019 [US1] Implement the read-only overview rendering in
      ...
      (depends on T018)
- [x] T020 [US1] Implement the empty state in the same
...
- [x] T021 [US1] Replace routing/exports: update
      ...
      T019, T020)
- [x] T022 [P] [US1] Component tests in
...
- [x] T023 [US2] Extend `account-overview.service.ts` (frontend) with `create()`, `update(id,
...
- [x] T024 [US2] Implement the add/edit modal in
...
- [x] T025 [US2] Wire the "Add account" toolbar button and each account row's edit icon (from
...
- [x] T026 [US2] Wire each account row's delete icon to a PrimeNG `ConfirmDialog` prompt before
      calling `account-overview.service.ts`'s `remove()` (FR-005, research.md #4), refreshing the
      list on success (depends on T023, T025)
- [x] T027 [US2] On successful add/edit save, refetch or locally patch the list so the overview
      reflects the change immediately with no manual refresh (SC-003) (depends on T023, T025)
- [x] T028 [P] [US2] Component tests for `account-overview-form.component.ts` in
...
- [x] T029 [P] [US2] Backend e2e test in `apps/backend/src/tests/account-overview.e2e-spec.ts`
...
- [x] T030 [P] [US3] Component test in `account-overview-page.component.spec.ts` (extends T022):
      ...
      (depends on T022)
- [x] T031 [P] [US3] Component test in `account-overview-page.component.spec.ts` (extends T022):
      ...
      T022, T027)
- [x] T032 [US3] Manually validate quickstart.md's Scenario 3 (grouping and the uncategorized
...
- [x] T033 [P] Run `npm exec nx run domain-accounts:lint`, the backend's lint target, and the
      frontend domain library's lint target; fix any violation
- [x] T034 Run `npm exec nx affected -t test` (or the equivalent explicit project list covering
...
- [x] T035 Execute quickstart.md's Scenario 1 (empty state) and Scenario 2 (add/view/edit/delete)
