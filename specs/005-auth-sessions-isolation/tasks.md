...
- [x] T001 Add `argon2` and `@nestjs/throttler` to `apps/backend/package.json` (or root
      `package.json` per existing dependency convention) and install via `pnpm install`
- [x] T002 [P] Generate `libs/domain/auth` Nx library (`pnpm nx g @nx/js:lib domain-auth --directory=libs/domain/auth`, matching `libs/domain/holdings`'s existing structure/tsconfig)
- [x] T003 [P] Create `apps/backend/src/auth/` directory with `auth.module.ts` placeholder (empty
      `@Module({})`, wired into `apps/backend/src/app.module.ts` imports) so subsequent tasks have
      a target module
- [x] T004 [P] Create `apps/frontend/src/app/auth/` directory placeholder for sign-in
...
- [x] T005 [P] Write failing unit tests for password-policy validation (8–200 chars; empty,
      too-short, too-long, boundary 8 and 200, valid) in
      `libs/domain/auth/src/lib/password-policy.spec.ts`
- [x] T006 [P] Write failing unit tests for lockout-delay calculation (threshold 5, geometric
      escalation 30s/60s/120s/... capped at 15 min, resets on success) in
      `libs/domain/auth/src/lib/lockout-policy.spec.ts`
- [x] T007 [P] Implement password-policy validation in `libs/domain/auth/src/lib/password-policy.ts`
      to make T005 pass (depends on T005)
- [x] T008 [P] Implement lockout-delay calculation in `libs/domain/auth/src/lib/lockout-policy.ts`
      to make T006 pass — pure function `(failedAttempts: number) => { locked: boolean; delaySeconds?: number }` (depends on T006)
- [x] T009 Export `password-policy.ts` and `lockout-policy.ts` from `libs/domain/auth/src/index.ts`
      (depends on T007, T008)

### Shared DTOs (`libs/api-contract`)

- [x] T010 [P] Create `SignInRequest`, `SessionUser`, `AuthErrorResponse` interfaces in
...
- [x] T011 Add `users` table creation (`CREATE TABLE IF NOT EXISTS users ...` with columns/CHECKs
...
- [x] T012 Add `sessions` table creation (id, user_id REFERENCES users(id), created_at,
...
- [x] T013 Add `holdings.owner_id` migration: `PRAGMA table_info(holdings)` check →
...
- [x] T015 [P] Write failing unit/integration tests for `UsersRepository`
...
- [x] T016 [P] Write failing unit/integration tests for `SessionsRepository` (create, findById,
...
- [x] T017 Implement `UsersRepository` in `apps/backend/src/auth/users.repository.ts` to make T015
      pass (depends on T015, T011)
- [x] T018 Implement `SessionsRepository` in `apps/backend/src/auth/sessions.repository.ts`,
...
- [x] T019 [P] Create `@Public()` decorator in `apps/backend/src/auth/public.decorator.ts`
- [x] T020 [P] Create `@Roles()` decorator in `apps/backend/src/auth/roles.decorator.ts`
- [x] T021 [P] Create `@CurrentUser()` param decorator in
      `apps/backend/src/auth/current-user.decorator.ts` (reads `request.user`)
- [x] T022 Implement `AuthGuard` in `apps/backend/src/auth/auth.guard.ts`: reads session cookie,
      ...
      T019, T021)
- [x] T023 Implement `RolesGuard` in `apps/backend/src/auth/roles.guard.ts`: reads `@Roles()`
      metadata and `request.user.role`, 403s on mismatch (depends on T020, T021)
- [x] T024 Register `AuthGuard` as a global guard via `APP_GUARD` in
...
- [x] T025 [P] [US1] Integration test: `POST /api/auth/sign-in` with valid bootstrap-admin
...
- [x] T026 [P] [US1] Integration test: sign-in → cookie → `GET /api/auth/session` returns
...
- [x] T027 [P] [US1] Integration test: `POST /api/auth/sign-out` → 204 + cookie cleared; subsequent
      request with the same stale cookie → 401 (FR-003, SC-003) in
      `apps/backend/src/auth/auth.controller.spec.ts`
- [x] T028 [P] [US1] Integration test: 6 consecutive wrong-password attempts on one account →
...
- [x] T029 [P] [US1] Integration test: a session past `SESSION_INACTIVITY_TIMEOUT_MINUTES` (or
...
- [x] T031 [US1] Implement `AuthController` in `apps/backend/src/auth/auth.controller.ts`:
...
- [x] T032 [US1] Wire `@nestjs/throttler` as a secondary global per-IP rate limit on
      `POST /api/auth/sign-in` in `apps/backend/src/auth/auth.module.ts` / `app.module.ts`
      (research.md #3) (depends on T024)
- [x] T033 [US1] Add cookie parsing (`cookie-parser` or Nest's built-in) and session cookie
...
- [x] T034 [US1] Register `AuthController`, `AuthService`, `UsersRepository`, `SessionsRepository`
...
- [x] T035 [P] [US1] Implement `AuthService` (`signIn`, `signOut`, `getSession`) in
      `apps/frontend/src/app/auth/auth.service.ts` calling the `/api/auth/*` endpoints, typed via
      `libs/api-contract`
- [x] T036 [P] [US1] Implement `SignInComponent` (email/password form, generic error display, calls
      `AuthService.signIn`) in `apps/frontend/src/app/auth/sign-in/sign-in.component.ts`
- [x] T037 [US1] Implement functional `authGuard` (calls `GET /api/auth/session`, redirects to
      `/sign-in` on failure) in `apps/frontend/src/app/auth/auth.guard.ts` (depends on T035)
- [x] T038 [US1] Implement `authInterceptor` (redirects to `/sign-in` on any 401 response) in
      `apps/frontend/src/app/auth/auth.interceptor.ts`, register in
      `apps/frontend/src/app/app.config.ts`
- [x] T039 [US1] Add `/sign-in` route (public) and apply `authGuard` to existing protected routes in
...
- [x] T040 [P] [US2] Integration test: as User A, `POST /api/holdings` creates a holding; as User B,
...
- [x] T041 [P] [US2] Integration test: as User B, `GET /api/holdings/:idOwnedByA` → 404 (not 403);
...
- [x] T042 [P] [US2] Integration test: User A edits/deletes their own holding; User B's holdings/
      views are unaffected (Acceptance #2) in `apps/backend/src/holdings/holdings.controller.spec.ts`
- [x] T043 [P] [US2] Integration test: any aggregate/summary endpoint reflects only the requesting
      user's holdings (Acceptance #3) in `apps/backend/src/holdings/holdings.controller.spec.ts`
- [x] T044 [P] [US2] Integration test: pre-existing (migrated) holdings from the bootstrap-admin
...
- [x] T045 [P] [US2] Integration test: signed in as an `ADMIN`, reading another user's holding by ID
      → 404, confirming role never implies cross-user access (Acceptance #5) in
      `apps/backend/src/holdings/holdings.controller.spec.ts`
- [x] T046 [P] [US2] Integration test: every `holdings` response body (list, detail, export) across
      the above tests contains no `owner_id`/ownership field (FR-010) in
      `apps/backend/src/holdings/holdings.controller.spec.ts`

### Implementation for User Story 2

- [x] T047 [US2] Add `ownerId` parameter and `AND owner_id = $N` predicate to every
      ...
      (depends on T013)
- [x] T048 [US2] Update `HoldingsController` to read `request.user.id` (via `@CurrentUser()`) and
...
- [x] T049 [US2] Audit and update any aggregate/dashboard/export endpoints in
...
- [x] T050 [P] Add structured logging (via existing `JsonLoggerService`) for sign-in
      success/failure/lockout events, logging account id (never the password) in
      `apps/backend/src/auth/auth.service.ts` (Principle V)
- [x] T051 [P] Update root `README.md` / deployment docs with `BOOTSTRAP_ADMIN_EMAIL`,
...
- [x] T052 Run the full `quickstart.md` validation (Scenarios A and B) manually against
      `docker compose up --build` and confirm every step's exact status code/body
- [x] T053 [P] Run `pnpm nx run-many -t lint test` across affected projects
