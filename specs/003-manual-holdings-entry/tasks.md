...
- [ ] T001 Generate the `domain-holdings` Nx library at `libs/domain/holdings` mirroring
...
- [ ] T002 [P] Confirm/add the `decimal.js` dependency to `libs/domain/holdings/package.json`
...
- [ ] T003 [P] Add `chart.js` and PrimeNG's `p-chart` module as a new frontend runtime dependency
      (`apps/frontend/package.json` / workspace root, per research.md #6 — the one new dependency
      this feature introduces)
- [ ] T004 [P] Create the `libs/api-contract/src/lib/holdings.ts` file (empty shell + export from
...
- [ ] T005 [P] Define `AssetType` union and per-type required/optional field metadata in
      `libs/domain/holdings/src/lib/asset-type.ts`, per data-model.md's AssetType table (FR-001)
- [ ] T006 [P] Write `libs/domain/holdings/src/lib/holding-validation.spec.ts` covering every rule
...
- [ ] T007 [US-shared] Implement `libs/domain/holdings/src/lib/holding-validation.ts` (including the
...
- [ ] T008 [P] Write `libs/domain/holdings/src/lib/holding-merge.spec.ts` covering data-model.md's
...
- [ ] T009 Implement `libs/domain/holdings/src/lib/holding-merge.ts` (pure decision function: given
...
- [ ] T010 Export `AssetType`, `Holding`, validation, and merge functions from
      `libs/domain/holdings/src/index.ts`; run `pnpm nx test domain-holdings` and confirm all specs
      pass (depends on T005, T007, T009)
- [ ] T011 [P] Populate `libs/api-contract/src/lib/holdings.ts` with `AssetType`, `HoldingResponse`,
...
- [ ] T012 Add the `holdings` table migration (the exact DDL in data-model.md's "Persistence"
...
- [ ] T013 [P] Scaffold the NestJS `holdings` feature module: `apps/backend/src/holdings/
    holdings.module.ts`, register it in `apps/backend/src/app/app.module.ts` alongside
      `DatabaseModule`/`HealthModule`
- [ ] T014 Implement `apps/backend/src/holdings/holdings.mapper.ts` (DB row ↔ domain `Holding` ↔
      `HoldingResponse`/`CreateHoldingRequest` DTO mapping, decimal string ↔ `Decimal` conversion)
      (depends on T007, T011)
- [ ] T015 Implement `apps/backend/src/holdings/holdings.repository.ts` (raw `pg` queries: list all,
...
- [ ] T016 Implement `apps/backend/src/holdings/holdings.service.ts` orchestrating domain validation
...
- [ ] T017 Implement `apps/backend/src/holdings/holdings.controller.ts` exposing
      ...
      (depends on T016)
- [ ] T018 Create the shell of `apps/backend/src/tests/holdings.e2e-spec.ts` (supertest against a
...
- [ ] T019 [P] [US1] Add `POST /holdings` contract test cases to
      ...
      FR-011, FR-011a)
- [ ] T020 [P] [US1] Add `POST /holdings` validation-failure cases to `holdings.e2e-spec.ts`: each
...
- [ ] T021 [US1] Wire `POST /holdings` end-to-end (controller → service → merge decision →
      repository, already scaffolded in Phase 2) until T019/T020 pass; run
      `pnpm nx test backend` to confirm
- [ ] T022 [P] [US1] Create `apps/frontend/src/app/holdings/holdings.service.ts` — `HttpClient`
      wrapper for `POST /holdings` (and `GET`, reused by US2) against the shared
      `libs/api-contract` DTOs
- [ ] T023 [US1] Create the add/edit form scaffold `apps/frontend/src/app/holdings/holding-form/
    holding-form.component.ts/html/css`: Angular Reactive Form, `p-select` for asset type (add
      ...
      T005, T022)
- [ ] T024 [US1] Implement per-type field-set switching in `holding-form.component.ts`: selecting a
...
- [ ] T025 [US1] Implement client-side validation feedback in `holding-form.component.ts/html`
...
- [ ] T026 [US1] Wire an "Add holding" action in `apps/frontend/src/app/holdings/
...
- [ ] T027 [P] [US1] Write `apps/frontend/src/app/holdings/holding-form/
...
- [ ] T028 [P] [US2] Add `GET /holdings` contract test cases to `holdings.e2e-spec.ts`: empty
...
- [ ] T029 [US2] Wire `GET /holdings` end-to-end (controller → service → repository, already
      scaffolded in Phase 2) until T028 passes
- [ ] T030 [US2] Implement the holdings list in `apps/frontend/src/app/holdings/
...
- [ ] T031 [US2] Implement the empty state in `holdings.component.html` (distinct from
...
- [ ] T032 [P] [US2] Create `apps/frontend/src/app/holdings/holdings-distribution/
...
- [ ] T033 [US2] Embed `holdings-distribution` in `holdings.component.html`, fed by the same fetched
      list as the table, hidden/empty-safe when there are no valued holdings (depends on T030,
      T032)
- [ ] T034 [P] [US2] Write `apps/frontend/src/app/holdings/holdings.component.spec.ts` covering:
...
- [ ] T035 [P] [US3] Add `PUT /holdings/:id` contract test cases to `holdings.e2e-spec.ts`: create a
...
- [ ] T036 [US3] Wire `PUT /holdings/:id` end-to-end (controller → service → repository — direct
      edit by id, no upsert-lookup, `assetType` immutable and not accepted in the body) until T035
      passes
- [ ] T037 [US3] Add an edit-mode entry point (row action in `holdings.component.html`) that opens
      `holding-form` pre-filled with the selected holding's values, asset type locked/read-only
      (FR-008, depends on T030)
- [ ] T038 [US3] Extend `holding-form.component.ts` to support edit mode: only the holding's own
...
- [ ] T039 [US3] Implement cancel in `holding-form.component.ts/html`: closes the dialog without
      calling the update endpoint, leaving the holding's stored values unchanged (FR-015, depends
      on T038)
- [ ] T040 [P] [US3] Extend `holding-form.component.spec.ts` with edit-mode cases: pre-filled
...
- [ ] T041 [P] [US4] Add `DELETE /holdings/:id` contract test cases to `holdings.e2e-spec.ts`:
...
- [ ] T042 [US4] Wire `DELETE /holdings/:id` end-to-end (controller → service → repository, hard
      delete) until T041 passes
- [ ] T043 [US4] Add a delete action (row action in `holdings.component.html`) that opens a
...
- [ ] T044 [US4] On successful delete, remove the holding from the in-memory list; on decline,
      leave the list unchanged (FR-016 Acceptance Scenario 2, depends on T043)
- [ ] T045 [US4] Handle a `404` response from the delete call as a non-error success path: treat as
...
- [ ] T046 [P] [US4] Extend `holdings.component.spec.ts` with delete cases: confirm-then-remove,
...
- [ ] T047 [P] Re-run `/speckit-ux-review` (or manually reconcile design.md) against the final
...
- [ ] T048 [P] Review structured logging added in T016 against Principle V — confirm every
      create/update/delete logs id, asset type, management, and outcome (created vs. updated vs.
      deleted)
- [ ] T049 Run the full quickstart.md manual/exploratory validation checklist (all 12 items) against
      a running local stack
- [ ] T050 Run `pnpm nx test domain-holdings`, `pnpm nx test backend`, `pnpm nx test frontend`, and
