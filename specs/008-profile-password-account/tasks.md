...
- [x] T001 [P] Add `ProfileSummary`, `UpdateDisplayNameRequest`, `RequestEmailChangeRequest`,
...
- [x] T002 [P] Add `EMAIL_CHANGE_EXPIRY_HOURS` (default `24`) and `PASSWORD_RESET_EXPIRY_HOURS`
...
- [x] T003 Add `migrateProfile()` to `apps/backend/src/database/database.service.ts`: idempotent
...
- [x] T004 [P] Integration test for `migrateProfile()` against a real temp-file SQLite (column/
      table/index existence, `CHECK` constraints, FK) in
      `apps/backend/src/database/database.service.spec.ts` (extend existing file)
- [x] T005 [P] Create `libs/domain/auth/src/lib/account-action-token.ts`: pure helpers
...
- [x] T006 [P] Unit tests for `isTokenUsable`/`expiryWindowHours` (all status/expiry combinations)
      in `libs/domain/auth/src/lib/account-action-token.spec.ts`
- [x] T007 Create `apps/backend/src/profile/account-action-tokens.repository.ts`:
...
- [x] T008 [P] Integration tests for `AccountActionTokensRepository` against a real temp-file
...
- [x] T009 [P] Add `updateDisplayName(id, displayName)`, `updateEmail(id, email)` (also clears
...
- [x] T010 [P] Unit/integration tests for the five new `UsersRepository` methods in
      `apps/backend/src/auth/users.repository.spec.ts` (extend existing file)
- [x] T011 [P] Add `deleteAllForUserExcept(userId, exceptSessionId)` to
...
- [x] T012 [P] Integration test for `deleteAllForUserExcept` (excluded session survives, all
      others removed) in `apps/backend/src/auth/sessions.repository.spec.ts` (extend existing file)
- [x] T013 Create `apps/backend/src/profile/profile.module.ts` wiring
...
- [x] T014 [US1] Add `GET /api/profile` handling to `ProfileService`: `getProfile(userId)` maps
...
- [x] T015 [US1] Add `updateDisplayName(userId, displayName)` to `ProfileService`: validates
...
- [x] T016 [US1] Create `apps/backend/src/profile/email.service.ts` with
...
- [x] T017 [US1] Add `requestEmailChange(userId, newEmail)` to `ProfileService`: calls
...
- [x] T018 [US1] Add `cancelEmailChange(userId)` to `ProfileService`: supersedes the user's pending
...
- [x] T019 [US1] Add `lookupEmailChangeToken(token)` to `ProfileService`: `findByTokenAndPurpose(
token, 'EMAIL_CHANGE')`, collapses not-found/wrong-status/expired into one `invalid_token` result
      (SC-002) via `isTokenUsable` (T005)
- [x] T020 [US1] Add `confirmEmailChange(token)` to `ProfileService`: re-validates via the guarded
...
- [x] T021 [US1] Create `apps/backend/src/profile/profile.controller.ts` with `GET /api/profile`
...
- [x] T022 [P] [US1] Unit tests for `ProfileService.getProfile/updateDisplayName/
...
- [x] T023 [P] [US1] Controller tests for the six routes in
      `apps/backend/src/profile/profile.controller.spec.ts`
- [x] T024 [P] [US1] Create `apps/frontend/src/app/settings/profile/profile.service.ts` wrapping
      ...
      from T001)
- [x] T025 [US1] Create Profile sub-tab in
...
- [x] T026 [US1] Register `app-profile` as the first `p-tab`/`p-tabpanel` ("Profile") in
...
- [x] T027 [US1] Add a `.role-badge` `<span>` bound to `user()?.role` in
...
- [x] T028 [US1] Create verify-email landing page in
...
- [x] T029 [US1] Create shared `apps/frontend/src/app/account/link-invalid/link-invalid.component.ts`
...
- [x] T030 [US1] Register `account/verify-email/:token` and `account/link-invalid` routes in
...
- [x] T031 [US2] Add `sendPasswordReset(to, token)` to `apps/backend/src/profile/email.service.ts`
      (extends T016's file)
- [x] T032 [US2] Add `changePassword(userId, currentSessionId, currentPassword, newPassword)` to
...
- [x] T033 [US2] Add `requestPasswordReset(email)` to `ProfileService`: always performs the same
...
- [x] T034 [US2] Add `lookupPasswordResetToken(token)` to `ProfileService`:
...
- [x] T035 [US2] Add `confirmPasswordReset(token, newPassword)` to `ProfileService`:
...
- [x] T036 [US2] Add `POST /api/profile/password` (T032), `@Public() POST
...
- [x] T037 [P] [US2] Unit tests for `ProfileService.changePassword/requestPasswordReset/
...
- [x] T038 [P] [US2] Controller tests for the four new routes in
      `apps/backend/src/profile/profile.controller.spec.ts` (extend T023's file)
- [x] T039 [US2] Integration test: two active sessions for one user, password change via session
...
- [x] T040 [US2] Integration test: forgot-password for an existing vs. nonexistent address produces
      byte-identical response bodies (extends T037)
- [x] T041 [US2] Integration test: a `PASSWORD_RESET` token never satisfies an `EMAIL_CHANGE`
...
- [x] T042 [P] [US2] Add `changePassword`/`requestPasswordReset`/`lookupResetToken`/
      `confirmPasswordReset` methods to `apps/frontend/src/app/settings/profile/profile.service.ts`
      (extends T024's file)
- [x] T043 [US2] Add password card to `profile.component.ts`/`.html` (extends T025): current/new/
...
- [x] T044 [US2] Create `apps/frontend/src/app/account/forgot-password/forgot-password.component.ts`
...
- [x] T045 [US2] Create `apps/frontend/src/app/account/reset-password/reset-password.component.ts`
...
- [x] T046 [US2] Register `account/forgot-password` and `account/reset-password/:token` routes in
...
- [x] T047 [US3] Add `deleteAccount(userId)` to `ProfileService`: delegates entirely to
...
- [x] T048 [US3] Add `DELETE /api/profile/account` to `ProfileController` (extends T036's file):
...
- [x] T049 [P] [US3] Unit tests for `ProfileService.deleteAccount` (success, last-admin-blocked,
      delegates-not-duplicates assertion) in `apps/backend/src/profile/profile.service.spec.ts`
      (extend T022's file)
- [x] T050 [P] [US3] Controller tests for the new route in
      `apps/backend/src/profile/profile.controller.spec.ts` (extend T023's file)
- [x] T051 [US3] Integration test: a `MEMBER` account (previously blocked by
...
- [x] T052 [US3] Integration test: sole active admin is blocked (`409`), then succeeds once a
      second admin exists, reusing 006's `canRemoveLastAdmin` fixture pattern — extends T049
- [x] T053 [P] [US3] Add `deleteAccount()` to `profile.service.ts` (frontend, extends T024/T042)
- [x] T054 [US3] Add Danger Zone card to `profile.component.ts`/`.html` (extends T025/T043): red-
...
- [x] T055 [P] Run quickstart.md's full walkthrough (Scenarios A, B, C) against a local dev stack
      with a test SMTP catcher; fix any deviation found
- [x] T056 [P] Update `apps/backend/src/app/app.module.ts` doc comment / root `README.md` (if
      either enumerates feature modules) to list `profile/`, matching how `invitations/`/
      `signups/`/`accounts/` are already documented
- [x] T057 Run `npm exec nx affected -t lint test build` (or `npm exec nx run-many -t lint test
